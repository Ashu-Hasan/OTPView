...
class OTPTextView : FrameLayout {
    ...
    private fun generateViews(styles: TypedArray, attrs: AttributeSet?) {
        ...
        if (length > 0) {
            ...
            val width = styles.getDimension(R.styleable.OtpTextView_width, OTPUtils.getPixels(context, DEFAULT_WIDTH).toFloat()).toInt()
            val height = styles.getDimension(R.styleable.OtpTextView_height, OTPUtils.getPixels(context, DEFAULT_HEIGHT).toFloat()).toInt()
            val space = styles.getDimension(R.styleable.OtpTextView_box_margin, OTPUtils.getPixels(context, DEFAULT_SPACE).toFloat()).toInt()
            val spaceLeft = styles.getDimension(R.styleable.OtpTextView_box_margin_left, OTPUtils.getPixels(context, DEFAULT_SPACE_LEFT).toFloat()).toInt()
            val spaceRight = styles.getDimension(R.styleable.OtpTextView_box_margin_right, OTPUtils.getPixels(context, DEFAULT_SPACE_RIGHT).toFloat()).toInt()
            val spaceTop = styles.getDimension(R.styleable.OtpTextView_box_margin_top, OTPUtils.getPixels(context, DEFAULT_SPACE_TOP).toFloat()).toInt()
            val spaceBottom = styles.getDimension(R.styleable.OtpTextView_box_margin_bottom, OTPUtils.getPixels(context, DEFAULT_SPACE_BOTTOM).toFloat()).toInt()
