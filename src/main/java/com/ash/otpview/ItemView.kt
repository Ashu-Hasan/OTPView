...
class ItemView : FrameLayout {
    ...
    private fun generateViews(styles: TypedArray) {
        val defaultHeight = OTPUtils.getPixels(context, DEFAULT_BAR_HEIGHT).toFloat()
        val defaultOtpTextSize = OTPUtils.getPixels(context, DEFAULT_OTP_TEXT_SIZE).toFloat()
        ...
        val barMargin = styles.getDimension(R.styleable.OtpTextView_bar_margin, OTPUtils.getPixels(
            context,
            0
        ).toFloat())
